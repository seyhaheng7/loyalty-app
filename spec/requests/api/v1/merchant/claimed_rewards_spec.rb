...
describe 'ClaimedRewards' do
  ...
  let!(:reward)  { create(:reward, quantity: 100, store: store) }
  let!(:claimed_reward1){ create(:claimed_reward, reward: reward1 ) }
  ...
  context 'GET api/v1/merchant/claimed_rewards' do
    ...
    let!(:claimed_reward1){ create(:claimed_reward, reward: reward1 ) }
    let!(:claimed_reward2){ create(:claimed_reward, reward: reward1 ) }
    let!(:claimed_reward3){ create(:claimed_reward, reward: reward1 ) }
    let!(:claimed_reward4){ create(:claimed_reward, reward: reward2 ) }
    ...
    it 'list of claimed_reward for merchant' do
    ...
    it 'not list of claimed_reward of other merchant' do
      json  = JSON.parse(response.body)
      ids   = json.map{ |j| j['id'] }
  ...
  describe 'Patch api/v1/merchant/claimed_rewards/given' do
    ...
    let!(:claimed_reward1){create(:claimed_reward, reward: reward, qr_token: 'fdkewidsjfwe3', customer: customer )}
    let!(:claimed_reward2){create(:claimed_reward, reward: reward, qr_token: 'fdkewidsjfwe1', customer: customer )}
    let!(:claimed_reward3){create(:claimed_reward, reward: reward, customer: customer )}
    let!(:claimed_reward4){create(:claimed_reward, reward: reward, qr_token: 'fdkewidsjfwedsaf', customer: customer )}
    ...
    it 'update given to true' do
      put given_api_v1_merchant_claimed_rewards_path(qr_token: claimed_reward3.qr_token ), headers: merchant.create_new_auth_token
  ...
  context 'GET api/v1/merchant/claimed_rewards?given=true' do
    let!(:given_claimed_reward)    { create(:claimed_reward, given: true, reward: reward1 ) }
    let!(:not_given_claimed_reward){ create(:claimed_reward, reward: reward1 ) }
